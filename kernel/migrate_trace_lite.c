/* 2015-04-07: File added and changed by Sony Corporation */
/*
...
static void *mstat_start(struct seq_file *m, loff_t *pos)
{
	...
	end_index = div64_u64((mstat_stop_sched_clock - mstat_start_sched_clock), period_nsec);
...
static int mstat_show(struct seq_file *m, void *v)
{
	...
	if (period == 0) {
		...
		seq_printf(m, "# period_msec %lld\n",
			   div64_u64(period_nsec, NSEC_PER_MSEC));
	...
	sec  =  div64_u64(period_ts, NSEC_PER_SEC);
#define MOD_BROKEN
#ifdef MOD_BROKEN
	msec = div64_u64((period_ts - (sec * NSEC_PER_SEC)), NSEC_PER_MSEC);
#else
	msec = div64_u64((period_ts % NSEC_PER_SEC), NSEC_PER_MSEC);
#endif
...
#define MSTAT_LOCATION(LOC)                                                    \
	if (mstat_enable) {                                                    \
		...
		now = div64_u64((sc_now - mstat_start_sched_clock), period_nsec);        \
									       \
		if (now < MAX_PERIOD) {                                        \
