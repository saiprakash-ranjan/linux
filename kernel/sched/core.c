...
void save_process_affinity(void)
{
	struct task_struct *g, *p;

	read_lock(&tasklist_lock);
	do_each_thread(g, p) {
		cpumask_copy(&p->cpus_allowed_saved, &p->cpus_allowed);
	} while_each_thread(g, p);
	read_unlock(&tasklist_lock);
}

void restore_process_affinity(void)
{
	struct task_struct *g, *p;
	struct task_struct *targets[16];
	int i, err, num_targets;
	...
	do {
		...
		read_lock(&tasklist_lock);
		do_each_thread(g, p) {
			if (!cpumask_empty(&p->cpus_allowed_saved) &&
			    !cpumask_equal(&p->cpus_allowed, &p->cpus_allowed_saved)) {
				get_task_struct(p);
				targets[num_targets++] = p;
				if (num_targets == ARRAY_SIZE(targets))
					goto out; /* cannot simply break */
			}
			else
				cpumask_clear(&p->cpus_allowed_saved);
		} while_each_thread(g, p);
	out:
		read_unlock(&tasklist_lock);

		/*
		 * we do this here because we cannot call set_cpus_allowed_ptr()
		 * in atomic context
		 */
		for (i = 0; i < num_targets; i++) {
			err = set_cpus_allowed_ptr(targets[i],
						   &targets[i]->cpus_allowed_saved);
			cpumask_clear(&targets[i]->cpus_allowed_saved);
			put_task_struct(targets[i]);

			if (err)
				printk(KERN_WARNING "PID %u: failed to restore affinity\n",
				       task_pid_nr(targets[i]));
#ifdef CONFIG_SCHED_DEBUG
			else
				printk(KERN_DEBUG "PID %u: restored affinity\n",
				       task_pid_nr(targets[i]));
#endif
		}
	} while (num_targets == ARRAY_SIZE(targets));

	put_online_cpus();
}
#endif /* CONFIG_SNSC_SAVE_PROCESS_AFFINITY_ON_SUSPEND */
