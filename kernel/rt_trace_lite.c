/* 2013-07-10: File added and changed by Sony Corporation */
/*
...
#if defined(CONFIG_ARM64) && defined(CONFIG_SPARSE_IRQ)
#define NR_IRQS_LITE 1020  /* Max number of interrupt sources GIC supports */
#else
#define NR_IRQS_LITE NR_IRQS_EXT
#endif

DEFINE_PER_CPU(struct irq_stat[NR_IRQS_LITE], lite_irq_stat);
	...
	if (bin_index == 0) {
		...
		if (entry->no_eip)
			...
			seq_printf(m,"#Average %-16s: %8llu\n", entry->name,
				   (od->count == 0) ? 0 : div_u64(od->total, od->count));
...
static void irq_stat_reset(void)
{
	...
	for (cpu = 0; cpu < NR_CPUS; cpu++ ) {
		for (irq = 0; irq < NR_IRQS_LITE; irq++) {
...
#define PMCR_E                  QUADD_ARMV8_PMCR_E
#define PMCR_C                  QUADD_ARMV8_PMCR_C
...
/* Macros for mapping the functions */
#define read_pmcr(value) 		value=armv8_pmu_pmcr_read()
#define write_pmcr(value)  		armv8_pmu_pmcr_write(value)
#define write_pm_cnt_en_set(value) 	armv8_pmu_pmcntenset_write(value)
#define write_pm_cnt_en_clr(value) 	armv8_pmu_pmcntenclr_write(value)
#define write_pm_sle_r(value)  		armv8_pmu_pmselr_write(value)
#define read_pm_ccnt_r(value) 		value=armv8_pmu_pmccntr_read()
#define read_pmx_ev_cnt_r(value) 	value=armv8_pmu_pmxevcntr_read()
#define write_pmx_ev_cnt_r(value) 	armv8_pmu_pmxevcntr_write(value)
#define write_pmx_ev_type_r(value) 	armv8_pmu_pmxevtyper_write(value)


/*********************************************************************
************** Functions are from  armv8_pmu.c ***********************
**********************************************************************/
...
static void disable_all_counters(void)
{
	...
	armv8_pmu_pmcntenclr_write(QUADD_ARMV8_CCNT | (PMCNTENSET_CNT0 | PMCNTENSET_CNT1));
}
...
#elif (__LINUX_ARM_ARCH__ == 7 || defined(CONFIG_ARM64))
	...
	)
{
	 ...
	 * #defined as "trace_hardirqs_off(); preempt_count_add(0);"
	 */
...
void notrace preempt_count_add(unsigned int val)
{
	int cpu = raw_smp_processor_id();
	struct _off_data *pod = preempt_off_data + cpu;
...
void notrace preempt_count_sub(unsigned int val)
{
	unsigned long delta;
	int cpu = raw_smp_processor_id();
	struct _off_data *pod = preempt_off_data + cpu;
...
static __init int irqs_off_init(void)
{
 ...
 || defined(CONFIG_SNSC_5_RT_TRACE_WITH_PMU)
	...
	irqs_off_data_reset();
	entry = proc_create("irqs_off", 0, NULL, &proc_irqs_off_operations);
#endif

#if defined(CONFIG_SNSC_LITE_IRQSOFF) && defined(CONFIG_SNSC_LITE_PREEMPTOFF)
	irqs_preempt_off_entry.name = "irq/preempt  off";
	irqs_preempt_off_data_reset();
	entry = proc_create("irqs_preempt_off", 0, NULL, &proc_irqs_preempt_off_operations);
#endif

#if defined(CONFIG_SNSC_LITE_PREEMPTOFF)
	preempt_off_entry.name = "preempt      off";
	preempt_off_data_reset();
	entry = proc_create("preempt_off", 0, NULL, &proc_preempt_off_operations);
#endif
	...
	irq_stat_reset();
	entry = proc_create("irqs_time", 0, NULL, &proc_irqs_time_operations);

#endif

#if defined(CONFIG_SNSC_LITE_IRQSOFF_DEBUG_MARK_WITH_PMU) \
 || defined(CONFIG_SNSC_5_RT_TRACE_WITH_PMU)
	entry = proc_create("mark_with_pmu", 0, NULL, &proc_mark_with_pmu_operations);
#endif
...
static inline void print_cal_header(struct seq_file *m, struct _off_data *od, struct _off_entry *entry)
{
	if (od->count) {
		seq_printf(m,"#adjusted %-16s cpu %d min,avg,max,count : "
			...
			entry->cpu,
			od->min - cal_min,
			div_u64(od->total, od->count) - cal_avg,
			od->max - cal_max,
			od->count);
	} else {
		seq_printf(m,"#adjusted %-16s cpu %d min,avg,max,count : "
...
static __init void rt_debug_time_calibrate(void)
{
	...
	cal_avg = (od->count) ? (div_u64(od->total, od->count)) : 0;
	cal_min = od->min;
	cal_max = od->max;

	od = rt_debug_2_data + cpu;

	cal_enter_exit_avg = (od->count) ? (div_u64(od->total, od->count)) : 0;
...
static __init void rt_debug_init(void)
{
	...
	entry = proc_create("rt_debug_1", 0, NULL, &proc_rt_debug_1_operations);

	entry = proc_create("rt_debug_2", 0, NULL, &proc_rt_debug_2_operations);

	entry = proc_create("rt_debug_3", 0, NULL, &proc_rt_debug_3_operations);

	entry = proc_create("rt_debug_4", 0, NULL, &proc_rt_debug_4_operations);

	entry = proc_create("rt_debug_5", 0, NULL, &proc_rt_debug_5_operations);

	entry = proc_create("rt_debug_6", 0, NULL, &proc_rt_debug_6_operations);
}
