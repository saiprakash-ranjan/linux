				   ...
				   unsigned long caller)
{
	...
	cache_free_account(objp, cachep->object_size);
				...
				int req_size)
{
	...
	cache_alloc_account((void *)caller, objp, cachep->object_size, req_size);
	return objp;
}
#else
...
{
	void *ret = slab_alloc(cachep, flags, _RET_IP_, -1);
...
kmem_cache_alloc_trace(struct kmem_cache *cachep, gfp_t flags, size_t size)
{
	...
	ret = slab_alloc(cachep, flags, _RET_IP_, -1);
...
{
	void *ret = slab_alloc_node(cachep, flags, nodeid, _RET_IP_, -1);
				  ...
				  size_t size)
{
	...
	ret = slab_alloc_node(cachep, flags, nodeid, _RET_IP_, -1);
					  ...
					  unsigned long caller)
{
	...
	if (unlikely(ZERO_OR_NULL_PTR(cachep)))
		return cachep;
	ret = slab_alloc(cachep, flags, caller, size);
