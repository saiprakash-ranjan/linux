...
static int utf8s_to_utf16s_len(const u8 *s, int len, enum utf16_endian endian, wchar_t *pwcs, int max)
{
	u16 *op;
	int size;
	unicode_t u;
	     ...
	     struct nls_table *nls)
{
	...
	if (utf8) {
#ifdef CONFIG_SNSC_FS_VFAT_COMPARE_UNICODE
		*outlen = utf8s_to_utf16s_len(name, len, UTF16_HOST_ENDIAN, (wchar_t *)outname, 260);
#else
...
static
int vfat_mknod(struct inode *dir, struct dentry *dentry, umode_t mode, dev_t rdev)
{
	/* base : vfat_create() */
	struct super_block *sb = dir->i_sb;
	struct msdos_sb_info *sbi = MSDOS_SB(sb);
