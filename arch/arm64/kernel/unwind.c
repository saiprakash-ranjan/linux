...
/*
 * arch/arm64/kernel/unwind.c
 * Copyright (C) 2016 Sony corp.
 *
 * Based on arch/arm/kernel/unwind.c
 *
...
static int unwind_exec_pop_subset_r4_to_r13(struct unwind_ctrl_block *ctrl,
						unsigned long mask)
{
	unsigned long *vsp = (unsigned long *)ctrl->sp;
	int load_sp, reg = 4;
...
static int unwind_exec_insn(struct unwind_ctrl_block *ctrl)
{
	...
	if ((insn & 0xc0) == 0x00)
		...
		if (ret)
		   ...
		   (insn & 0x0d) != 0x0d)
		ctrl->sp = ctrl->vrs[insn & 0x0f];
	else if ((insn & 0xf0) == 0xa0) {
		ret = unwind_exec_pop_r4_to_rN(ctrl, insn);
		if (ret)
			goto error;
	} else if (insn == 0xb0) {
		// TODO: you cannot write PC like this on ARM64
		if (ctrl->pc == 0)
			ctrl->pc = ctrl->vrs[LR];
		...
		ctrl->sp += 0x204 + (uleb128 << 2);
	} else {
		pr_warn("unwind: Unhandled instruction %02lx\n", insn);
		return -URC_FAILURE;
	}

	if (compat_user_mode(ctrl)) {
		lr = ctrl->vrs[COMPAT_LR];
		fp = ctrl->vrs[COMPAT_FP];
	} else {
		lr = ctrl->vrs[LR];
		fp = ctrl->vrs[FP];
	}
	...
	pr_debug("%s: fp = %08lx sp = %08lx lr = %08lx pc = %08lx\n", __func__,
			 ctrl->vrs[fp], ctrl->sp, ctrl->vrs[lr], ctrl->pc);
...
void unwind_backtrace(struct pt_regs *regs, struct task_struct *tsk)
{
	...
	if (regs) {
		arm64_get_current_stackframe(regs, &frame);
		/* PC might be corrupted, use LR in that case. */
		if (!kernel_text_address(regs->pc))
...
EXPORT_SYMBOL_GPL(unwind_backtrace);
