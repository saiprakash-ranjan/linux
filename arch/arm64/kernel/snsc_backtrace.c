/* 2018-11-09: File added and changed by Sony Corporation */
/*
 * arch/arm64/kernel/snsc_backtrace.c
 *
 * Copyright (C) 2016 Sony corp.
 *
 * Based on arch/arm/kernel/snsc_backtrace.c
...
static inline unsigned long bt_get_user32(const unsigned long *addr)
{
	...
	register unsigned int kpc asm("x26");
	asm("adr x26, .");

	set_fs(KERNEL_DS);
	if (copy_from_user(&data, addr, sizeof(unsigned long))) {
		/* data is set to zero */
		printk("bt_get_user32: illegal address %p pc:%x ra:%p\n", addr, kpc, __builtin_return_address(0));
	}
	set_fs(oldfs);
	return data;
}
#ifdef CONFIG_ARM64_UNWIND_USER
 ...
 * address.
 *
 * Need down_write(&current->mm->mmap_sem).
 */
...
int bt_munmap(const void *addr, unsigned long len)
{
	...
	return do_munmap(current->mm, paged_addr, len + offset_in_page);
}
...
static int search_current_map_exec_addr(const char *needle, struct vm_area_struct **rvm)
{
	...
	for (; vm; vm = vm->vm_next) {
		if (vm->vm_file && vm->vm_file->f_path.dentry &&
			...
			if(strncmp(needle, vm->vm_file->f_path.dentry->d_name.name, strlen(needle))==0) {
...
#else
			ep->be_exidx_tab = unwind_table_add((unsigned long)ep->be_exidx,
#endif
					ep->be_exidx_size,
					vm->vm_start,
	...
	int ba_size, void **ba_buf, void *ba_skip_addr)
{
	...
	frame.sp = _regs->sp;
	frame.lr = _regs->regs[30];
	frame.pc = _regs->pc;
#ifdef CONFIG_ARM64_UNWIND
	for (i = 0; i < 31; i++) {
		ctrl.vrs[i] = _regs->regs[i];
	}
	ctrl.pc = _regs->pc;
	ctrl.sp = _regs->sp;
	ctrl.pstate = _regs->pstate;
#endif
	       ...
	       struct pt_regs *_regs, bt_callback_t *cb, void *uarg)
{
	...
	if (_regs) {
		...
		pr_debug("%s:%d: is atomic, ustack disabled in atomic\n", __func__, __LINE__);
#ifdef CONFIG_ARM64_UNWIND
		for (i = 0; i < 31; i++) {
			ctrl.vrs[i] = _regs->regs[i];
		}
		    ...
		    bt_callback_t *cb, void *uarg, int funtop_possible)
{
	...
	frame.lr = _regs->regs[30];
	frame.pc = _regs->pc;
	//Assiging pc = some value. is this possible in AARCH64
	pc = frame.pc;

	dump_backtrace(_regs, task);
}
		       ...
		       bt_callback_t *cb, void *uarg)
{
	...
	register unsigned long current_lr asm ("x30");
	register unsigned long kpc asm("x26");
	asm("adr x26, .");
	_regs.pc = kpc;
	_regs.sp= current_sp;
	_regs.regs[30] = current_lr;
	_regs.regs[29] = (unsigned long)__builtin_frame_address(0);
