...
enum mx6q_clks {
	...
	esai_extal, gpt_ipg, gpt_ipg_per, gpu2d_core, gpu3d_core, iahb,
	isfr, i2c1, i2c2, i2c3, iim, enfc, ipu1, ipu1_di0, ipu1_di1, ipu2,
...
#ifdef CONFIG_MACH_COWBOY
static void init_eim_slow_clk(void)
{
	u32 reg;
	/* CLKCTL_CCGR6: Set eim_slow clock to be on in all modes */
	reg = readl_relaxed(ccm_base + 0x80);
	reg |= 0x00000C00;
	writel_relaxed(reg, ccm_base + 0x80);
}
#endif
...
static void __init imx6q_clocks_init(struct device_node *ccm_node)
{
	...
	clk[iahb]         = imx_clk_gate2("iahb",          "ahb",               base + 0x70, 0);
	clk[isfr]         = imx_clk_gate2("isfr",          "pll3_pfd1_540m",    base + 0x70, 4);
	...
	imx_clk_set_parent(clk[ldb_di0_sel], clk[pll2_pfd0_352m]);
	imx_clk_set_parent(clk[ldb_di1_sel], clk[pll2_pfd0_352m]);
	init_ldb_clks();
...
#ifdef CONFIG_MACH_COWBOY
		init_eim_slow_clk();
		imx_clk_set_parent(clk[axi_alt_sel], clk[pll2_pfd2_396m]);
		if (clk_get_rate(clk[eim_slow]) != 99000000)
			printk(KERN_ERR "Warning: eim_slow clock not set to 99 MHz!"
	...
	imx_clk_set_rate(clk[spdif1_sel], 480000000);
