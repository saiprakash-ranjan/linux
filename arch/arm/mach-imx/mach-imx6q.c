...
static struct resource imx6q_pmu_resources[] = {
	[0] = {
		.start		= 126,
		.end		= 126,
...
static struct platform_device imx6q_pmu_device = {
	.name			= "arm-pmu",
	.id			= -1,
	.num_resources		= ARRAY_SIZE(imx6q_pmu_resources),
	.resource		= imx6q_pmu_resources,
};

static inline void  imx6q_add_armpmu(void)
{
	u32 val = 2;
	asm volatile("mcr p15, 0, %0, c1, c1, 1" : : "r" (val));

	platform_device_register(&imx6q_pmu_device);
}
