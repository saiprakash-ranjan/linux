...
static bool arm_dark_resume_check(struct list_head *list)
{
...
struct pm_dark_resume_ops dark_resume_ops = {
	.check = arm_dark_resume_check,
};

static __init int arm_dark_resume_init(void)
{
	pm_dark_resume_register_ops(&dark_resume_ops);
	return 0;
}
arch_initcall(arm_dark_resume_init);
