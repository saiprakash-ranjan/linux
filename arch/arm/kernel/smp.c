...
#ifdef CONFIG_SUPPORT_TEE_ASYNC_CMD
static tee_notify_handler_t *tee_notify_handler = NULL;
#endif
...
#ifdef CONFIG_SUPPORT_TEE_ASYNC_CMD
        S(IPI_TEE_NOTIFY - 1, "TEE notifications"),
#endif
};
...
#ifdef CONFIG_SUPPORT_TEE_ASYNC_CMD
int register_tee_notify_handler(tee_notify_handler_t handler)
{
	if (handler && !tee_notify_handler) {
		tee_notify_handler = handler;
		return 0;
	} else {
		return -EINVAL;
	}
...
int unregister_tee_notify_handler(tee_notify_handler_t handler)
{
	if (handler == tee_notify_handler) {
		tee_notify_handler = NULL;
		return 0;
	} else {
		return -EINVAL;
	}
}
EXPORT_SYMBOL(register_tee_notify_handler);
EXPORT_SYMBOL(unregister_tee_notify_handler);
#endif
