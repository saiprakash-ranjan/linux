...
#ifdef CONFIG_SNSC_SSBOOT
extern int  jetson_suspend(void *func);
extern void jetson_resume(void);
...
static int jetson_ssboot_prepare_pgd(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(lpmap_addr); i++) {
		...
		pmd_v = pmd_offset(pud_offset(pgd_offset(current->mm, virt), virt), virt);
		pmd_p = pmd_offset(pud_offset(pgd_offset(current->mm, phys), phys), phys);
...
static int jetson_ssboot_restore_pgd(void)
{
	pmd_t *pmd_p, *pmd_sv;
	int i;
...
static void jetson_save_state(void)
{
	jetson_ssboot_prepare_pgd();
}

static void jetson_restore_state(void)
{
	jetson_ssboot_restore_pgd();
}
#endif /* CONFIG_SNSC_SSBOOT */
...
#ifdef CONFIG_SNSC_SSBOOT
	ret = tegra_suspend_dram(state, current_suspend_mode, 0);
#else
...
#ifdef CONFIG_SNSC_SSBOOT
	/*
	 * From suspend_valid_for_ssboot(), currently tegra only support
	 * PM_SUSPEND_MEM and PM_SUSPEND_SNAPSHOT. Since we do not call this
	 * tegra_enter_lp0() fucntion in SnapshotBoot, so the suspend state must
	 * be PM_SUSPEND_MEM when tegra_enter_lp0() is called.
	 */
	tegra_suspend_dram(PM_SUSPEND_MEM, TEGRA_SUSPEND_LP0, 0);
#else
...
#ifdef CONFIG_SNSC_SSBOOT
int tegra_suspend_dram(suspend_state_t state, enum tegra_suspend_mode mode, unsigned int flags)
#else
...
#endif
{
	...
	if (state == PM_SUSPEND_SNAPSHOT) {
		jetson_save_state();
		jetson_suspend(ssboot_snapshot);
		jetson_restore_state();
	} else {
...
#ifdef CONFIG_SNSC_SSBOOT
        if(state == PM_SUSPEND_SNAPSHOT)
                tegra_cpu_reset_handler_init_ssboot();
#endif
...
#ifdef CONFIG_SNSC_SSBOOT
int suspend_valid_for_ssboot(suspend_state_t state)
{
	return state == PM_SUSPEND_SNAPSHOT || state == PM_SUSPEND_MEM;
}
#endif
...
#ifdef CONFIG_SNSC_SSBOOT
	.valid		= suspend_valid_for_ssboot,
#else
