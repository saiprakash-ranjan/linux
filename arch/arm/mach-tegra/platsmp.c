...
#ifdef CONFIG_SNSC_SSBOOT
	/*
	 * Nonboot cpu MUST be powered up by power gate at the first time.
	 * From the later on, flow controller can power on/off nonboot cpu.
	 * During cold boot, the flow controller is reset and not enabled,
	 * so before power up cpu by flow controller, need to check if it is
	 * enabled.
	 */
	is_flow_ctrl_enable = flowctrl_read_cpu_csr(cpu) & FLOW_CTRL_CSR_ENABLE;
	if (cpu_isset(cpu, tegra_cpu_power_map) && is_flow_ctrl_enable) {
#else
