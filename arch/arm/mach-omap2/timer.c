...
#ifdef CONFIG_SNSC_OMAP_USE_GPT_DEFAULT
static bool use_gptimer_clksrc = true;
#else
...
unsigned long long notrace snsc_raw_clock(void)
{
	if (use_gptimer_clksrc)
		return clocksource_cyc2ns(
			clocksource_gpt.read(&clocksource_gpt),
			clocksource_gpt.mult, clocksource_gpt.shift);
	else
		return omap_sync32k_raw_clock();
}

unsigned long long notrace snsc_raw_clock_get_res(void)
{
	if (use_gptimer_clksrc)
		return NSEC_PER_SEC / clksrc.rate;
	else
		return omap_sync32k_raw_clock_get_res();
}
