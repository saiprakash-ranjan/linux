/* 2013-10-31: File changed by Sony Corporation */
...
#ifdef CONFIG_SNSC_USB_STOR_WHITELIST
static char whitelist[128];
module_param_string(whitelist, whitelist, sizeof(whitelist), S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(whitelist,
		"Accept USB Mass Storage devices with specific VID/PIDs only");
#endif
...
#ifdef CONFIG_SNSC_USB_STOR_WHITELIST
/* Check device to be authorized by "whitelist=vid:pid" module parameter */
static void check_whitelist(struct us_data *us)
{
	char *p;
	u16 vid = le16_to_cpu(us->pusb_dev->descriptor.idVendor);
	u16 pid = le16_to_cpu(us->pusb_dev->descriptor.idProduct);
	unsigned f = 0;
	unsigned int mask = US_FL_IGNORE_DEVICE;

	p = whitelist;

	/* If whitelist is empty, accepts all USB mass storage devices */
	if (!*p)
		return;
