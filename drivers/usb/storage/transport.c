...
#if defined(CONFIG_SNSC_USB_STOR_PARAM_DID_ABORT_RETRY_MAX)
static unsigned int snsc_did_abort_retry_max = 1;
module_param(snsc_did_abort_retry_max, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(snsc_did_abort_retry_max, "DID_ABORT retry max");
#endif

#if defined(CONFIG_SNSC_USB_STORAGE_ADDITIONAL_UNUSUAL_DEVS)
#include "transport_unusual.c"
#endif
...
void usb_stor_invoke_transport(struct scsi_cmnd *srb, struct us_data *us)
{
	int need_auto_sense;
	int result;
#if defined(CONFIG_SNSC_USB_STOR_UEVENT_CMD_TIMEOUT)
	char env_retrycnt[24];
	char env_retrymax[24];
	char env_timeout[23];
	char env_opcode[15];
	char *envp[5];
#endif
	...
	if (test_bit(US_FLIDX_TIMED_OUT, &us->dflags)) {
...
#if defined(CONFIG_SNSC_USB_STOR_PARAM_DID_ABORT_RETRY_MAX)
		srb->allowed = snsc_did_abort_retry_max;
#endif
#if defined(CONFIG_SNSC_USB_STOR_UEVENT_CMD_TIMEOUT)
		snprintf(env_retrycnt, 24, "CMDRETRYCNT=%d", srb->retries);
		snprintf(env_retrymax, 24, "CMDRETRYMAX=%d", srb->allowed);
		snprintf(env_timeout, 23, "CMDTIMEOUT=%d",
			 srb->request->timeout / HZ);
		snprintf(env_opcode, 15, "CMDOPCODE=0x%02x", srb->cmnd[0]);
		...
		kobject_uevent_env(&srb->device->sdev_gendev.kobj,
				   KOBJ_CHANGE, envp);
#endif
		goto Handle_Errors;
	}
...
Retry_Sense:
...
#if defined(CONFIG_SNSC_USB_STORAGE_ADDITIONAL_UNUSUAL_DEVS)
		else if ((srb->sense_buffer[2] & 0xf) == MEDIUM_ERROR
			 || (srb->sense_buffer[2] & 0xf) == HARDWARE_ERROR
			) {
			usb_stor_dbg(us, "quit the command %x "
				  "because of sense %x/%x/%x\n",
				  srb->cmnd[0],
				  srb->sense_buffer[2] & 0xf,
				  srb->sense_buffer[12],
				  srb->sense_buffer[13]);
			srb->device->changed = 1;
			srb->result = DID_ERROR << 16;
			goto Handle_Errors;
		}
		else if (srb->cmnd[0] == TEST_UNIT_READY &&
			 (srb->sense_buffer[2] & 0xf) == ILLEGAL_REQUEST) {
			/* We may get unexpected sense key with TUR command,
                        * after the command was failed. (ex. MSAC-US20)
			 * WRITE_10 ... Key:5h, ASC=30h, ASCQ=5h
			 * READ_10  ... Key:5h, ASC=11h, ASCQ=0h */
			usb_stor_dbg(us, "unexpected sense data "
				  "with TEST_UNIT_READY\n");
			srb->device->changed = 1;
		}
		else if ((us->flags_exp & US_FL_SNSC_DETECTCHG_BY_SENSEDATA)&&
			 srb->cmnd[0] == WRITE_10 &&
			 (srb->sense_buffer[2] & 0xf) == 0xb &&
			 srb->sense_buffer[12] == 0 &&
			 srb->sense_buffer[13] == 0 ) {
			usb_stor_dbg(us, KERN_INFO "ABORTED_COMMAND WRITE_10\n");
			srb->device->changed = 1;
		}
		else if (srb->cmnd[0] == INQUIRY &&
			 (srb->sense_buffer[2] & 0xf) == NOT_READY &&
			 srb->sense_buffer[12] == 0x3a && /* ASC_MEDIUM_NOT_PRESENT */
			 srb->sense_buffer[13] == 0) {
			usb_stor_dbg(us, KERN_INFO "RETRY INQUIRY\n");
			if (srb->retries < srb->allowed)
				srb->result = DID_ERROR << 16;
		}
#endif
	}
	...
	if (result == USB_STOR_TRANSPORT_FAILED
	    && (srb->sense_buffer[2]&0xf) == ILLEGAL_REQUEST
	    && srb->sense_buffer[12] == 0x20) { /* ASC_INVALID_OPCODE */
		usb_stor_dbg(us, "usb_stor_invoke_transport:"
			  "ask scsi to retry because of ILLEGAL_REQUEST "
			  "scsi command=%x\n", srb->cmnd[0]);
		if (srb->retries < srb->allowed)
			srb->result = DID_ERROR << 16;
	}
...
#if defined(CONFIG_SNSC_USB_STORAGE_TRANSPORT_RESET_FIRST)
	usb_stor_report_device_reset(us);
#endif
	scsi_unlock(us_to_host(us));
#if defined(CONFIG_SNSC_USB_STORAGE_TRANSPORT_RESET_FIRST)
	/* According to BOT and CBI transport specifications,
	 * usb_stor_port_reset() should be used as the last means.
	 * First transport_reset() is tried here.
	 */
...
#if defined(CONFIG_SNSC_USB_STORAGE_ADDITIONAL_UNUSUAL_DEVS)
/*
 * do special handling for particular devices.
 * or just call original usb_stor_invoke_transport()
 */
void usb_stor_invoke_transport_wrapper(struct scsi_cmnd *srb,
					struct us_data *us)
{
