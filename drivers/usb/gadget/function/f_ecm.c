...
static inline unsigned ecm_bitrate(struct usb_gadget *g)
{
	if (gadget_is_superspeed(g) && g->speed == USB_SPEED_SUPER)
		/*
		 * NO clear description about SuperSpeed bulk transfer
		 * speed in USB3.0 spec, use isoc transfer speed instead.
		 */
		return 16 * 3 * 1024 * 8 * 1000 * 8;
...
#ifdef CONFIG_SNSC_USB_ETH_ENABLE_CHANGE_MAX_MTU_SIZE
	/* maximum segment size the LAN frame facility can support. WMC p.32 */
	.wMaxSegmentSize =	cpu_to_le16(USB_ETH_MAX_FRAME_LEN),
#else
