...
/* get the 2nd uvc_buffer in queue, used by uvc merge feature */
struct uvc_buffer *uvcg_queue_next_to_head(struct uvc_video_queue *queue)
{
	struct uvc_buffer *buf = NULL;

	if (list_empty(&queue->irqqueue) || list_is_singular(&queue->irqqueue))
		return NULL;

	buf = list_first_entry(&queue->irqqueue, struct uvc_buffer, queue);

	return list_next_entry(buf, queue);
}
