...
qh_urb_transaction (
	...
	/* check urb flags for single step control transfer */
	if ((urb->transfer_flags & (URB_SINGLE_STEP_CTRL_SETUP|
				   URB_SINGLE_STEP_CTRL_DATA|
				    URB_SINGLE_STEP_CTRL_STATUS))
	    && !usb_pipecontrol (urb->pipe)) {
		printk("%s:Can't execute single step.\n",__FUNCTION__);
		return NULL;
	}
...
#ifdef CONFIG_SNSC_USB_SINGLE_STEP_CTRL
	/* check setup stage skip*/
	if (urb->transfer_flags &
	    (URB_SINGLE_STEP_CTRL_DATA|URB_SINGLE_STEP_CTRL_STATUS)) {
		token ^= QTD_TOGGLE;
		goto data_stage;
	}
...
#ifdef CONFIG_SNSC_USB_SINGLE_STEP_CTRL
	/* check data stage skip. */
	if (urb->transfer_flags & URB_SINGLE_STEP_CTRL_STATUS)
		goto status_stage;
#endif /* CONFIG_SNSC_USB_SINGLE_STEP_CTRL */
...
#ifdef CONFIG_SNSC_USB_SINGLE_STEP_CTRL
	/* data stage qtds are prepared. */
	if (urb->transfer_flags & URB_SINGLE_STEP_CTRL_DATA)
		goto done;
...
#ifdef CONFIG_SNSC_USB_SINGLE_STEP_CTRL
		/* prepare status stage qtd. */
		if (one_more &&
		    urb->transfer_flags & URB_SINGLE_STEP_CTRL_STATUS) {
