...
#if defined(CONFIG_SNSC_USB_EHCI_OVERRIDE_PWRON2PWRGOOD)
	desc->bPwrOn2PwrGood = CONFIG_SNSC_USB_EHCI_PWRON2PWRGOOD/2; /* 2ms unit */
#else
...
static int ehci_hub_control (
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_TEST:
			...
			switch (selector) {
			case 5: /* Test FORCE_ENABLE */
				temp = ehci_readl(ehci, &ehci->regs->command);
				temp |= CMD_RUN;
				ehci_writel(ehci, temp, &ehci->regs->command);
				break;
			case 0: /* Test mode not enabled */
				ehci_reset(ehci);
				ehci_run(hcd);
				break;
			}
