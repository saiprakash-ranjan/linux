					...
					int bufNum, dma_addr_t phyaddr)
{
	...
	if ((ch == 23) || (ch == 24) || (ch == 27) || (ch == 28) || (ch == 29)) {
		/* ODD field first */
		if (g_ipu_hw_rev >= IPU_V3H) {
			scan_order = ipu_ch_param_read_field_io(ipu_ch_param_addr(ipu, ch), 0, 113, 1);
			if (scan_order)
				odd_first = 1;
		}
	}
...
static inline void _ipu_ch_param_set_interlaced_scan(struct ipu_soc *ipu, uint32_t ch)
{
	...
	if (odd_first) {
		ipu_ch_param_mod_field_io(ipu_ch_param_addr(ipu, ch), 1, 58, 20, ((~((unsigned int)(stride / 8))) + 1) & 0xfffff);
		if (sub_ch > 0)
			ipu_ch_param_mod_field_io(ipu_ch_param_addr(ipu, sub_ch), 1, 58, 20,
						   ((~((unsigned int)(stride / 8))) + 1) & 0xfffff);
	} else {
