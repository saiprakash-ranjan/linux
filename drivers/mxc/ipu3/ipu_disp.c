			    ...
			    uint32_t v_to_h_sync, ipu_di_signal_cfg_t sig)
{
	...
	uint32_t reg;
	uint32_t di_gen, vsync_cnt, apixel_cnt;
	...
	if (sig.interlaced) {
			...
			} else if (g_ipu_hw_rev >= IPU_V3DEX) {
			...
			_ipu_di_sync_config(ipu, disp, DI_SYNC_AFIELD,
					...
					DI_SYNC_INT_HSYNC, v_start_width + v_sync_width, DI_SYNC_INT_HSYNC, 2, DI_SYNC_VSYNC,
			...
			/* Field 0 VSYNC waveform */
			_ipu_di_sync_config(ipu, disp, DI_SYNC_COUNT_6, v_total - 1, DI_SYNC_INT_HSYNC,
					0, DI_SYNC_NONE,
			...
			/* active pixel waveform */
			apixel_cnt = DI_SYNC_COUNT_8;
			_ipu_di_sync_config(ipu, disp, DI_SYNC_COUNT_8, 0, DI_SYNC_CLK,
		...
		if (disp) {
			_ipu_dc_write_tmpl(ipu, 1, WROD(0), 0, map, SYNC_WAVE, 0, apixel_cnt, 1);
			if ((pixel_fmt == IPU_PIX_FMT_YUYV) ||
				(pixel_fmt == IPU_PIX_FMT_UYVY) ||
				(pixel_fmt == IPU_PIX_FMT_YVYU) ||
				(pixel_fmt == IPU_PIX_FMT_VYUY)) {
				_ipu_dc_write_tmpl(ipu, 8, WROD(0), 0, (map - 1), SYNC_WAVE, 0, apixel_cnt, 1);
				_ipu_dc_write_tmpl(ipu, 9, WROD(0), 0, map, SYNC_WAVE, 0, apixel_cnt, 1);
				/* configure user events according to DISP NUM */
				ipu_dc_write(ipu, (width - 1), DC_UGDE_3(disp));
			}
		} else {
			_ipu_dc_write_tmpl(ipu, 0, WROD(0), 0, map, SYNC_WAVE, 0, apixel_cnt, 1);
			if ((pixel_fmt == IPU_PIX_FMT_YUYV) ||
				(pixel_fmt == IPU_PIX_FMT_UYVY) ||
				(pixel_fmt == IPU_PIX_FMT_YVYU) ||
				(pixel_fmt == IPU_PIX_FMT_VYUY)) {
				_ipu_dc_write_tmpl(ipu, 10, WROD(0), 0, (map - 1), SYNC_WAVE, 0, apixel_cnt, 1);
				_ipu_dc_write_tmpl(ipu, 11, WROD(0), 0, map, SYNC_WAVE, 0, apixel_cnt, 1);
				/* configure user events according to DISP NUM */
				ipu_dc_write(ipu, width - 1, DC_UGDE_3(disp));
			}
		}
		...
		if (disp) {
			...
			_ipu_dc_write_tmpl(ipu, 2, WROD(0), 0, map, SYNC_WAVE, 8, apixel_cnt, 1);
			_ipu_dc_write_tmpl(ipu, 3, WROD(0), 0, map, SYNC_WAVE, 4, apixel_cnt, 0);
		   ...
		   _ipu_dc_write_tmpl(ipu, 5, WROD(0), 0, map, SYNC_WAVE, 8, apixel_cnt, 1);
		   _ipu_dc_write_tmpl(ipu, 6, WROD(0), 0, map, SYNC_WAVE, 4, apixel_cnt, 0);
		   _ipu_dc_write_tmpl(ipu, 7, WRG, 0, map, NULL_WAVE, 0, DI_SYNC_CLK, 1);
		   _ipu_dc_write_tmpl(ipu, 12, WROD(0), 0, map, SYNC_WAVE, 0, apixel_cnt, 1);
		}
