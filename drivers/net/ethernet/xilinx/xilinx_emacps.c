...
#define ZYNQ_MESSAGES_DEFAULT ( NETIF_MSG_DRV | NETIF_MSG_PROBE | \
...
static u32 xemacps_get_msglevel(struct net_device *ndev)
{
	struct net_local *lp = netdev_priv(ndev);
	return lp->msg_enable;
}

static void xemacps_set_msglevel(struct net_device *ndev, u32 level)
{
	struct net_local *lp = netdev_priv(ndev);
	lp->msg_enable = level;
}
...
static struct ethtool_ops xemacps_ethtool_ops = {
	...
	.get_msglevel   = xemacps_get_msglevel,
	.set_msglevel   = xemacps_set_msglevel,
...
static int xemacps_probe(struct platform_device *pdev)
{
	...
	lp->pdev = pdev;
	lp->ndev = ndev;
	lp->msg_enable = netif_msg_init(debug, ZYNQ_MESSAGES_DEFAULT);
