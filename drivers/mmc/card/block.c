...
#ifdef CONFIG_SNSC_MMC_BKOPS_ENABLE_IOCTL
static int ioctl_do_enable_bkops(struct mmc_card* card)
{
	int err;

	BUG_ON(!card);
	if (!card->ext_csd.bkops) {
			pr_warn("%s: %s - BKOPS is not supported\n",
				mmc_hostname(card->host), __func__);
			err = -EOPNOTSUPP;
			goto out;
	}

	err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_BKOPS_EN, 1,
		card->ext_csd.generic_cmd6_time);
	if (err)
		pr_err("%s: %s - EXT_CSD_BKOPS_EN failed. err=%d\n",
			mmc_hostname(card->host), __func__, err);
	else
		card->ext_csd.man_bkops_en = true;
...
#ifdef CONFIG_SNSC_MMC_BKOPS_ENABLE_IOCTL
	if ((MMC_EXTRACT_INDEX_FROM_ARG(cmd.arg) == EXT_CSD_BKOPS_EN) &&
	    (cmd.opcode == MMC_SWITCH)) {
		err = ioctl_do_enable_bkops(card);
		if (err)
			pr_err("%s: ioctl_do_enable_bkops() failed. err = %d",
			       __func__, err);
					      ...
					      int area_type)
{
	struct mmc_blk_data *md;
	int devidx, ret;
#ifdef CONFIG_SNSC_MMC_FIX_MINOR_NUMBER
	int idx;
	...
	if (idx < 0)
		...
		devidx = find_next_zero_bit(dev_use, max_devices, idx);
#else
	devidx = find_first_zero_bit(dev_use, max_devices);
#endif
	 ...
	 */
	if (!subname) {
#ifndef CONFIG_SNSC_MMC_FIX_MINOR_NUMBER
		int idx;
