/* 2017-07-24: File changed by Sony Corporation */
/*
...
#ifdef CONFIG_SNSC_SERIAL_DEMOLISH
	int dem = SNSC_SERIAL_DEMOLISH_NOTHING;
	int dem_port = -1;
#endif
...
int serial8250_console_read(struct console *co, char *buffer,
			    unsigned count)
{
	struct uart_8250_port *up = &serial8250_ports[co->index];
	unsigned int ier, i;
...
int serial8250_flush_serial_tty(struct console *co)
{
	...
	line = co->index;
	if (line >= ARRAY_SIZE(serial8250_ports))
		return -ENODEV;
	...
	while (tty->driver->ops->chars_in_buffer(tty)) {
		if (serial_in(uart, UART_LSR) & UART_LSR_THRE)
			serial8250_tx_chars(uart);
	}
	return 0;
}
...
static struct console univ8250_console = {
	...
	.flush_tty      = serial8250_flush_serial_tty,
#endif
...
#ifdef CONFIG_SNSC_SERIAL_DEMOLISH
module_param(dem, int, 0644);
MODULE_PARM_DESC(dem, "Demolish serial output for 8250 tegra devices");
#endif
