...
static int imx_console_read(struct console *co, char *buffer,
				   unsigned count)
{
	struct imx_port *sport = imx_ports[co->index];
	struct imx_port_ucrs old_ucr;
	unsigned int ucr1, status;
	...
	do {
		status = readl(sport->port.membase + USR2);

	} while (~status & USR2_RDR);

	/* read */
	*buffer++ = readl(sport->port.membase + URXD0);
