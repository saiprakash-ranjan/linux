...
static unsigned int xuartps_tx_empty(struct uart_port *port)
{
	...
	status = (xuartps_readl(XUARTPS_SR_OFFSET) & XUARTPS_SR_TXEMPTY);
...
static int xuartps_console_read(struct console *co, char *buffer,
                unsigned int count)
{
	...
	u32 imr;
        ...
        for(i = 0; i < count; i++){
	 	while((xuartps_readl(XUARTPS_SR_OFFSET)) == 0) {
                        udelay(1);
                }
	    	*buffer++ = xuartps_readl(XUARTPS_FIFO_OFFSET);
        }
        /*
         *      restore the IER
         */
