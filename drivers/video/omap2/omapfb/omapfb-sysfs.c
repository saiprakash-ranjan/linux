...
#ifdef CONFIG_SNSC_FB_OMAP2_SYSFS_SUPPORT_TIMINGS
...
static ssize_t show_support_timings(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fb_info *fbi = dev_get_drvdata(dev);
	...
	for (i = 0; i < specs->modedb_len; ++i) {
		...
		if (r == 0) {
			buff_len += snprintf(buf + buff_len, PAGE_SIZE,
					"%u,%u/%u/%u/%u,%u/%u/%u/%u\n",
					t.pixel_clock,
...
#ifdef CONFIG_SNSC_FB_OMAP2_SYSFS_SUPPORT_TIMINGS
	__ATTR(support_timings, S_IRUGO,
			show_support_timings, NULL),
#endif
