...
struct vivi_dev {
	...
	u8			   lines_yuv420[3][MAX_WIDTH * 8] __attribute__((__aligned__(4)));
...
static void precalculate_line_yuv420(struct vivi_dev *dev)
{
	int plane_no;
	int colorpos;
	int width, wstart, wend, w;

	for (plane_no = 0; plane_no < 3; plane_no++) {
		width = (plane_no == 0) ? dev->width : dev->width / 2;

		for (colorpos = 0; colorpos < 16; colorpos++) {
			wstart = colorpos * width / 8;
			wend = (colorpos + 1) * width / 8;

			for (w = wstart; w < wend; w++) {
				dev->lines_yuv420[plane_no][w] =
					dev->bars[colorpos % 8][plane_no];
			}
		}
	}
}
...
static void vivi_fillbuff_yuv420(struct vivi_dev *dev, struct vivi_buffer *buf)
{
	void *vbuf = vb2_plane_vaddr(&buf->vb, 0);
	int plane_no, width, height, mv_count, h;
	u8 *linestart;
	...
	for (plane_no = 0; plane_no < 3; plane_no++) {
		width = (plane_no == 0) ? dev->width : dev->width / 2;
		height = (plane_no == 0) ? dev->height : dev->height / 2;
		mv_count = (plane_no == 0) ? dev->mv_count : dev->mv_count / 2;

		linestart = dev->lines_yuv420[plane_no] + mv_count % width;

		for (h = 0; h < height; h++)
			memcpy(vbuf + h * width, linestart, width);

		vbuf = (char *)vbuf + width * height;
	}
...
static void vivi_thread_tick(struct vivi_dev *dev)
{
	...
	/* Fill buffer */
	if (dev->fmt->fourcc == V4L2_PIX_FMT_YUV420)
		vivi_fillbuff_yuv420(dev, buf);
	else
				...
				unsigned int sizes[], void *alloc_ctxs[])
{
	...
	if (fmt)
		...
		size = dev->width * dev->height * dev->fmt->depth / 8;
					...
					struct v4l2_format *f)
{
	...
	/*
	 * for packed formats, bytesperline is usually calculated as
	 *     width times pixelsize.
	 * for planar formats, bytesperline only applies to the 1st plane,
	 *     so it should just be the width of 1st plane.
	 */
	f->fmt.pix.bytesperline = dev->fmt->fourcc == V4L2_PIX_FMT_YUV420 ?
		f->fmt.pix.width : f->fmt.pix.width * dev->pixelsize;
	/*
	 * for packed formats, sizeimage is usually calculated as
	 *     height times bytesperline.
	 * for planar formats, since bytesperline only applies to the 1st plane,
	 *     sizeimage should add up sizes of all planes.
	 */
	f->fmt.pix.sizeimage    = dev->fmt->fourcc == V4L2_PIX_FMT_YUV420 ?
		f->fmt.pix.height * f->fmt.pix.width * 3 / 2 :
