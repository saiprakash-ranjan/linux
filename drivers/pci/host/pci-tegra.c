/* 2017-07-09: File changed by Sony Corporation */
/*
...
#define NV_PCIE2_RP_PRIV_XP_LCTRL_2				0xF6C
#define RP_PRIV_XP_LCTRL_2_DEEMPHASIS_STRAP			(1 << 19)
...
static void tegra_pcie_set_deemphasis(struct tegra_pcie_port *port)
{
	u32 val;

	val = rp_readl(port, NV_PCIE2_RP_PRIV_XP_LCTRL_2);
	val |= RP_PRIV_XP_LCTRL_2_DEEMPHASIS_STRAP;
	rp_writel(port, val, NV_PCIE2_RP_PRIV_XP_LCTRL_2);
}
...
static void tegra_pcie_enable_features(struct tegra_pcie *pcie)
{
	...
	list_for_each_entry(port, &pcie->ports, list) {
		...
		if (port->set_deemphasis)
			tegra_pcie_set_deemphasis(port);
	}
}
...
static int tegra_pcie_parse_dt(struct tegra_pcie *pcie)
{
	...
	for_each_child_of_node(np, port) {
		...
		rp->pcie = pcie;
		rp->np = port;
		rp->set_deemphasis = of_property_read_bool(port,
			"nvidia,deemphasis-strap-3.5db");
