		...
		enum pon_power_off_type type)
{
	...
	dev_err(&pon->pdev->dev, "ps_hold power off type = 0x%02X\n", type);
	return rc;
}
		...
		enum pon_power_off_type type)
{
	...
	dev_err(&pon->pdev->dev, "resin power off type = 0x%02X\n", type);
	return rc;
}
...
static int qpnp_pon_config_init(struct qpnp_pon *pon)
{
	...
	if (!pon->num_pon_config) {
		dev_err(&pon->pdev->dev, "num_pon_config: %d\n",
			pon->num_pon_config);
		return 0;
	}
...
static int pon_spare_regulator_enable(struct regulator_dev *rdev)
{
	...
	pr_info("reg %s enable addr: %x bit: %d\n", rdev->desc->name,
...
static int pon_spare_regulator_disable(struct regulator_dev *rdev)
{
	...
	pr_info("reg %s disable addr: %x bit: %d\n", rdev->desc->name,
		pon_reg->addr, pon_reg->bit);
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	pr_info("PON@SID %d: num_pon_config: %d num_pon_reg: %d\n",
		to_spmi_device(pon->pdev->dev.parent)->usid,
	...
	pr_info("%s: pon_subtype=%x, pon_version=%x\n", __func__,
