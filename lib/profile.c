 ...
 *
 * Copyright 2005-2007,2015 Sony Corporation
 *
...
static int sched_clock_proc_write(struct file *file, const char __user *buffer,
...
static int sched_clock_proc_read(struct file *file, char __user *buf,
				 size_t count, loff_t *ppos)
{
	unsigned tlen;
...
static struct file_operations proc_sched_clock_operations = {
	.read   = sched_clock_proc_read,
	.write  = sched_clock_proc_write,
};

static int __init debug_proc_sched_clock(void)
{
	struct proc_dir_entry *ent;
	ent = proc_create("sched_clock", S_IFREG|S_IRUGO|S_IWUSR, NULL,
			  &proc_sched_clock_operations);
	if(!ent) {
...
static int profile_record_proc_write(struct file *file, const char __user *buffer,
				  size_t count, loff_t *ppos)
{
	if(down_interruptible(&record_index_sem))
		return -EINTR;
...
static int profile_record_proc_read(struct file *file, char __user *buf,
				    size_t count, loff_t *ppos)
{
...
static struct file_operations proc_prof_record_operations = {
	.read   = profile_record_proc_read,
	.write  = profile_record_proc_write,
};
...
static int __init debug_proc_profile(void)
{
	...
	ent = proc_create("profile_record", S_IFREG|S_IRUGO|S_IWUSR, NULL,
			  &proc_prof_record_operations);
	if (!ent) {
