/* 2015-06-19: File added and changed by Sony Corporation */
/*
 * lib/heap_profile.c
 *
...
#ifndef CONFIG_SNSC_DEBUG_PROFILE_HEAP_RECORDS_NUM
#define CONFIG_SNSC_DEBUG_PROFILE_HEAP_RECORDS_NUM  7000
#endif

#define  RECORDS CONFIG_SNSC_DEBUG_PROFILE_HEAP_RECORDS_NUM
...
static profile_heap_record_t profile_heap_records[RECORDS];
...
static int heap_record_to_string(profile_heap_record_t *record, char *buf, int len)
{
	...
	used = totalram_pages - record->nr_free_pages - record->nr_cache_pages;
	...
	return snprintf(buf,len,
			...
			(record->req_pages + used) << (PAGE_SHIFT - 10),
...
static int profile_heap_proc_read(struct file *file, char __user *buf,
...
static int profile_heap_proc_write(struct file *file, const char __user *buffer,
				 size_t count, loff_t *ppos)
{
	if(down_interruptible(&heap_record_index_sem))
		return -EINTR;
...
static struct file_operations proc_heap_operations = {
	.read	= profile_heap_proc_read,
	.write	= profile_heap_proc_write,
};
